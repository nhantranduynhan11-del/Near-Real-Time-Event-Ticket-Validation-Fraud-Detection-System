...
| **F02 (Ticket Validation):** Kiểm tra mã vé tồn tại, đúng sự kiện, đúng cổng được chỉ định và trạng thái hiệu lực (`UNUSED`). | Phân quyền truy cập nâng cao (RBAC) chi tiết cho từng loại tài khoản nhân viên. | Xác thực sinh trắc học hoặc nhận diện khuôn mặt tại cổng. |
...
flowchart TD
    ...
    E -->|Trạng thái CANCELLED| F2[scan_result: CANCELLED]
    E -->|Trạng thái EXPIRED hoặc quá giờ| F3[scan_result: EXPIRED]
    ...
    E -->|Vé hợp lệ ban đầu - UNUSED| G{Kiểm tra lịch sử các lần quét}
    ...
    J1 --> K1[Cập nhật trạng thái vé:<br>FLAGGED_FRAUD]
    
    I2 -->|Entry: YES| J2[Cho phép qua cổng<br>Cập nhật trạng thái vé: VALID_ENTRY]
```
...
1. **Tính hoàn chỉnh (End-to-End):**
   * **Khởi tạo dữ liệu (Pre-event):** Danh sách vé được nạp vào hệ thống qua chức năng `F09 (Import Valid Ticket List) / UC06` trước khi sự kiện mở cửa, khởi tạo trạng thái ban đầu là `UNUSED`.
   ...
   * **Xử lý trung gian (Processing & Logic):** Kiểm tra tính hợp lệ cơ bản, ghi log kiểm toán (`F06 Audit Logging`), và đối chiếu lịch sử quét để phát hiện gian lận (`F03 Fraud Detection`).
   * **Điểm cuối (Output):** Phản hồi tức thì về thiết bị soát vé tại cổng theo đúng tập kết quả chuẩn (`VALID`, `INVALID`, `USED`, `CANCELLED`, `EXPIRED`, `WRONG_GATE`), cập nhật trạng thái vé trong cơ sở dữ liệu (`VALID_ENTRY` hoặc `FLAGGED_FRAUD`), đồng thời đẩy cảnh báo tức thời lên giao diện của `Event Supervisor` (`F07 Live Monitoring Dashboard`).

2. **Tính độc lập:**
   * Mọi chức năng cốt lõi bắt buộc (`F01` đến `F09`) đều hoạt động độc lập, không bị phụ thuộc vào bất kỳ tính năng mở rộng nào ở cột **Nice-to-Have** hay **Out of Scope**.

3. **Tính kế thừa và nguồn gốc:**
   * Danh mục mã chức năng từ `F01` đến `F09` khớp 100% với tài liệu `03_Functions.md` và ánh xạ đầy đủ `UC01` đến `UC06` của Phase 1.
   * Vòng đời trạng thái vé đồng bộ hoàn toàn với bộ trạng thái tại `06_Ticket_Lifecycle.md` (`UNUSED`, `VALID_ENTRY`, `FLAGGED_FRAUD`, `CANCELLED`, `EXPIRED`).
